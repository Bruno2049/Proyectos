...
namespace eClockSync
{
    class CeClockTerm : CeTerminalSync
    {
        ...
        {
            AgregaLog("PERSONAS_A_VEC_T1 " + CeC.Convierte2String(Fila.PERSONAS_A_VEC_T1));
            string IDEmpleado = CeC.Convierte2Int(Fila.PERSONAS_A_VEC_T1).ToString();
            return IDEmpleado;
        ...
        protected override Errores EnvioVectores()
        {
            ...
            foreach (WS_eCheck.DS_WSPersonasTerminales.DT_PersonasTerminalesRow Fila in m_DatosEmpleados)
            {
                try
                {
                    ...
                    AgregaLog(" Prueba Log " + IDEmpleado);
