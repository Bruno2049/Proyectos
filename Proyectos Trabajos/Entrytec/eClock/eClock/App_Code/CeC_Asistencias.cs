...
public class CeC_Asistencias
{
    ...
    private static int GeneraNoAsistencia()
    {
        ...
        foreach (DataRow DR in DSPersonas.Tables[0].Rows)
        {
            ...
            int AccesoIDEntrada = CeC_Accesos.AgregarAcceso(PersonaID, 0, 2, Fecha.Add(HoraEntrada));
            int AccesoIDSalida = CeC_Accesos.AgregarAcceso(PersonaID, 0, 3, Fecha.Add(HoraSalida));
    ...
    public static bool EnviaMail(string EMail, string Titulo, string Texto)
    {
        ...
        return CeC_BD.EnviarMail(EMail, Titulo, Cuerpo);
    }
    /// <summary>
    ...
    public static bool EnviaMail(int Persona_ID, string Titulo, string Texto)
    {
        ...
        try
        {
            ...
            CeC_BD.EnviarMail(Email, Titulo, Cuerpo);
            return true;
    ...
    public static int CalculaDia(DS_CEC_Asistencias.EC_PERSONAS_DIARIORow RPersona_Diario, DS_CEC_Asistencias.EC_TURNOS_DIARow RTurnos_Dia, bool EnviarMail)
    {
        try
        {
            ...
            {
                CIsLog2.AgregaLog("Calculo HorasExtras Dia");
            ...
            {
                if (EsDiaFestivo(RPersona_Diario.PERSONA_DIARIO_FECHA) > 0)
                {
...
" AND DIA_SEMANA_ID =" + Cont);
    ...
    /// <returns></returns>
    public static int EsDiaFestivo(DateTime Dia)
    {
        return CeC_BD.EjecutaEscalarInt("select DIA_FESTIVO_ID from eC_dias_Festivos where dia_festivo_borrado = 0 and DIA_FESTIVO_FECHA = " + CeC_BD.SqlFecha(Dia));
    }
    /// <summary>
    /// 
    /// </summary>
    ...
    public static int ProcesaSoloAsistencia(int Acceso_ID, int Persona_ID, DateTime FechaHora)
    {
        ...
        int DiaFestivo = EsDiaFestivo(FechaHora);
        if (DiaFestivo > 0)
        {
    ...
    /// <summary>
    /// Valida si la agrupacion contiene el formato especificado.
    /// </summary>
    /// <param name="PersonasIDs"></param>
    ...
    /// <returns></returns>
    public static DataSet ObtenAsistenciaHorizontalN(bool EntradaSalida, bool TurnoDia, bool IncidenciaAbr, bool ColorTurno, bool ColorIncidencia, bool MuestraAgrupacion, bool MuestraEmpleado, int Persona_ID, string Agrupacion, DateTime FechaInicial, DateTime FechaFinal, string Lang, CeC_Sesion Sesion)
    {
        string Campos = "";
        string OrdenarPor = "";
    ...
    public static DataSet ObtenAsistenciaTotalesHistorial(bool MostrarAgrupacion, bool MostrarEmpleado, int Persona_ID, string Agrupacion, DateTime FechaInicial, DateTime FechaFinal, CeC_Sesion Sesion)
    {
        ...
        if (Campos.Length > 0)
        {
            ...
            Qry += " ORDER BY " + OrdenarPor;
        }

        DataSet DS = (DataSet)EjecutaDataSet(Qry, Sesion);
        //DataSet DS = (DataSet)CeC_BD.EjecutaDataSet(Qry);
    ...
    public static DataSet ObtenAsistenciaTotalesSaldos(bool MostrarAgrupacion, bool MostrarEmpleado, int Persona_ID, string Agrupacion, DateTime FechaInicial, DateTime FechaFinal, CeC_Sesion Sesion)
    {
        ...
        Campos = CeC.AgregaSeparador(Campos, "ALMACEN_INC_SALDO", ", ");
        string Qry = "SELECT   " + Campos +
...
" ";
    ...
    public static DataSet ObtenAsistenciaTotales(bool MostrarAgrupacion, bool MostrarEmpleado, bool Totales, bool Historial, bool Saldos, bool Faltas, int Persona_ID, string Agrupacion, DateTime FechaInicial, DateTime FechaFinal, CeC_Sesion Sesion)
    {
        ...
        CamposTot = CeC.AgregaSeparador(Campos, "COUNT(*) as Totales", ", ");
        CamposTot = CeC.AgregaSeparador(CamposTot, "MAX(INCIDENCIA_ID) as Saldo", ", ");
        ...
        foreach (DataRow DR in DS.Tables[0].Rows)
        {
            if (CeC.Convierte2Int(DR["Saldo"]) == -9999)
            {
                DR["Saldo"] = CeC_IncidenciasInventario.ObtenSaldo(CeC.Convierte2String(DR["INCIDENCIA_NOMBRE"]), Persona_ID, Sesion.USUARIO_ID, Agrupacion);
            }
        }
    ...
    public static DataSet ObtenAsistenciaGrafica(bool MostrarAgrupacion, bool MostrarEmpleado, bool MostrarFecha, int Persona_ID, string Agrupacion, DateTime FechaInicial, DateTime FechaFinal, CeC_Sesion Sesion)
    {
        ...
        return (DataSet)EjecutaDataSet(Qry, Sesion);
        //return (DataSet)CeC_BD.EjecutaDataSet(Qry);
    }
    ...
    public static DataSet EjecutaDataSet(string Qry, CeC_Sesion Sesion)
    {
        if (Sesion == null)
            return (DataSet)CeC_BD.EjecutaDataSet(Qry);
        CeC_SesionBD SBD = new CeC_SesionBD(Sesion.SESION_ID);
        string QryHash = CeC_BD.CalculaHash(Qry);
        DataSet DS;
        if (Sesion.UltimoReporteQryHash != QryHash || (DateTime.Now - Sesion.UltimoReporteQryFechaHora) > new TimeSpan(0, 0, CeC_Config.MinutosGuardaDataSet, 0))
        {
            Sesion.UltimoReporteQryHash = QryHash;
            DS = (DataSet)CeC_BD.EjecutaDataSet(Qry);
            SBD.UltimoReporteDS = DS;
        }
        else
            DS = SBD.UltimoReporteDS;
        Sesion.UltimoReporteQryFechaHora = DateTime.Now;
        return DS;
    }
    ...
    public static DataSet ObtenAsistenciaLinealN(int PERSONA_DIARIO_ID_INICIO, int PERSONA_DIARIO_ID_FIN, string Lang, int Usuario_ID)
    {
        ...
        Campos += ",TURNO_DIA_PHEX AS PHEX,TURNO_DIA_HAYCOMIDA AS PCOMI";
...
" ORDER BY PERSONA_DIARIO_ID";
    ...
    public static string ObtenFechas(string PersonasDiario)
    {
        DataSet DS = (DataSet)CeC_BD.EjecutaDataSet("SELECT PERSONA_LINK_ID, PERSONA_DIARIO_FECHA FROM EC_PERSONAS_DIARIO,EC_PERSONAS WHERE EC_PERSONAS_DIARIO.PERSONA_ID = EC_PERSONAS.PERSONA_ID AND PERSONA_DIARIO_ID IN (" + PersonasDiario + ")");
        if (DS == null || DS.Tables.Count < 1 || DS.Tables[0].Rows.Count < 1)
            return "";
        string Ret = "";
    ...
    {
        DataSet DS = (DataSet)CeC_BD.EjecutaDataSet("SELECT PERSONA_ID FROM EC_PERSONAS_DIARIO WHERE PERSONA_DIARIO_ID IN (" + PersonasDiarioIDs + ") GROUP BY PERSONA_ID");
        if (DS == null || DS.Tables.Count < 1 || DS.Tables[0].Rows.Count < 1)
            return "";
        string Ret = "";
        foreach (DataRow DR in DS.Tables[0].Rows)
        {
            int PersonaID = CeC.Convierte2Int(DR["PERSONA_ID"]);
            Ret = CeC.AgregaSeparador(Ret, PersonaID.ToString(), ",");
        }
        return Ret;
    }
