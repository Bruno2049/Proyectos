...
function igedit_number(elem,id,prop0,prop1)
{
	...
	me.setValue=function(v,noFix)
	{
		this.text=this.toTxt(this.value=this.toNum(v,true,true),this.fcs==2);
		this.repaint();
		this.select(1000);
		if(this.fix==1&&noFix!=2)this.old=this.instant(true);
	}
...
function igedit_date(elem,id,prop0,prop1)
{
	...
	me._isPM=function(t,i)
	{
		var pm=this.getAMPM().toUpperCase();
	...
	me.toDate=function(t,foc,limit,fire)
	{
		...
		else if(n<30)
		{
			...
			if(day>0||d.getDate()>28)d.setDate(10);if(y>=0)d.setFullYear(y);if(mo>0)d.setMonth(mo-1);if(day>0)d.setDate(day);
		}
		n&=15;
	...
	me.fields0=function(t)
	{
		...
		while(++i<m.length)
		{
			...
			else
			{
				...
				else if(n!=x&&this._isPM(t.toUpperCase(),j))
					v=1;
			}
			n=x;
		}
...
function igedit_new(elem,id,prop0)
{
	...
	this.doKey0=function(e,a)
	{
		...
		var meta=false,ctrl=e.ctrlKey,paste=a==2&&(k==17||(ig_shared.IsMac&&(k==91||k==224)));
		if(ig_shared.IsMac)ctrl=meta=e.metaKey;
		if(a!=1&&(meta||e.altKey||k==17||paste))
		{
			if(e.altKey)this.k0=-1;
			else if(t0!=t1&&(k==86||paste)){this.paste(t1);this._np=1;}
