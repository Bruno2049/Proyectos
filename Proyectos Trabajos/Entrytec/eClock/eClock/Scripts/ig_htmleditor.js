...
function iged_new(id,ta,tb,p1,p2,p3,p4,p5)
{
	...
	this._pos=function(evt,pan,flag)
	{
		...
		while(ee&&ee!=e0)
		{
			...
			if(this._ie&&(ee.nodeName=='TD'||ee.nodeName=='TABLE'||!this._ie7)){x+=ee.offsetLeft;y+=ee.offsetTop;}
			if(ee.offsetParent)ee=(p==e0)?e0:ee.offsetParent;
			else ee=p;
		}
		ee=elem;
		while(saf&&ee&&ee!=e0&&ee.nodeName!='BODY')
		{
			x1+=ee.offsetLeft;y1+=ee.offsetTop;
			ee=ee.offsetParent;
		}
	...
	try
	{
		...
		this._ie7=id.indexOf('msie 6.')>0||id.indexOf('msie 7.')>0;
	...
	this._onPaste=function()
	{
		if(!this._ie)try{
			if(navigator.userAgent.toLowerCase().indexOf('firefox')>=0)
				setTimeout(function(){iged_act('cleanword');},10);
		}catch(ex){}
	}
...
function iged_act(key,p1,p2,p3,p4,p5)
{
	...
	switch(act)
	{
		...
		case "findreplace":if(o._ie)return;
			i=o._elem.contentWindow;
			if(!i.nsFindInstData)i.nsFindInstData=new Function();
			i.find();
			break;
		case "pop":
			var p6=null;
			...
			break;
		case "popwin":o._findOld=null;o._popWin(p1,p2,p3,p4,p5);break;
		...
		case "finddialog":if(o._find0(1)==5)return;break;
		case "replacedialog":if(o._find0(2)==5)return;break;
		case "replacealldialog":if(o._find0(3)==5)return;break;
