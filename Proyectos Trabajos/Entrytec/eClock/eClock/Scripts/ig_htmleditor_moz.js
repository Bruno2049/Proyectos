...
function iged_init(ids,p1,p2,p3,p4,p5)
{
	...
	o._find0=function(x)
	{
		if(!this._saf)return 5;
		var s=null,txt=iged_el('iged_f_fw').value,txt2=iged_el('iged_f_rw').value||'',
			mc=iged_el('iged_f_mc').checked,up=iged_el('iged_f_mw').checked,doc=this._cDoc();
		...
		while(!s)
		{
			...
			this._findNum++;
			this._findOld=null;
			if(x==2&&this._win().find(txt,mc,up))
			{
				this._findOld=txt;
				break;
			}
		}
		if(s)alert(this._cs(s).replace('{0}',txt));
	}
	var d=o._doc();
	if(!elem._oldE)
	{
