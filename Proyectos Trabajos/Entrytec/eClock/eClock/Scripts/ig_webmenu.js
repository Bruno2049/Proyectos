...
function igmenu_menu(menuElement,menuProps) {
	...
	this.displaySubMenu = function(subMenu, parentItem, vertical) {
        ...
        var submenuFirstChild = ig_shared.getFirstNoneTextChild(subMenu);
		...
		if(subMenu.style.width == "") {
			...
			if(ig_csom.IsNetscape6 || ig_csom.IsMac) {
				...
				if(ig_csom.IsIE55Plus || ig_csom.IsIE9) {
					if(submenuFirstChild != null) {
						submenuFirstChild.style.width = subMenu.style.width;
						if(submenuFirstChild.firstChild != null)
							submenuFirstChild.firstChild.style.width = subMenu.style.width;
					}
				}
			}
		}
		...
		if(ig_csom.IsIE  && !ig_csom.IsMac) {
			var scrollHeight, 
                firstSubMenuItem = ig_shared.getFirstNoneTextChild(subMenu);

            if(firstSubMenuItem == null)
			    return;
			if(!subMenu.scroller)
		        scrollHeight = firstSubMenuItem.scrollHeight; 
            else
	...
	this.cellFromRow = function(item) {
		if(ig_csom.IsIE) {
			if(item.tagName == "TR") {
                return ig_shared.getFirstNoneTextChild(item);
			}
            else  {
				return item;
            }
