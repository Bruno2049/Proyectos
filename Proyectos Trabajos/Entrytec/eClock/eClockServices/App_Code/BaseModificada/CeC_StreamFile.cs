...
/// <summary>
/// Descripción breve de CeC_StreamFile
/// </summary>
public class CeC_StreamFile : eClockBase.CeC_Stream
{
    ...
    public override string ObtenPath(string ArchivoNombre)
    {
        ...
        return HttpContext.Current.Server.MapPath(System.IO.Path.Combine(Path, ArchivoNombre));
    }
    ...
    public override bool NuevoTexto(string ArchivoNombre, string Texto)
    {
        try
        {
            if (System.IO.Path.GetExtension(ArchivoNombre) == ".Lst")
                return false;
            ArchivoNombre = ObtenPath(ArchivoNombre);
            if (System.IO.File.Exists(ArchivoNombre))
                System.IO.File.Delete(ArchivoNombre);
