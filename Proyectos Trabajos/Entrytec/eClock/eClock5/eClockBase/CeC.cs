...
namespace eClockBase
{
    ...
    public class CeC
    {
        ...
        public static string ObtenDiasTexto(List<DateTime> Dias)
        {
            var DiasOrd = from D in Dias
                          orderby D
                          select D;
            string Resultado = "";

            foreach (var DiaOrd in DiasOrd)
            {
                Resultado = AgregaSeparador(Resultado, DiaOrd.ToString("d"), ",");
            }
            return Resultado;
        }

        public static string Json2Csv(string CadenaJson)
        {
            var JSon = Newtonsoft.Json.JsonConvert.DeserializeObject(CadenaJson);
            string Csv = "";
            Newtonsoft.Json.Linq.JContainer Contenedor = (Newtonsoft.Json.Linq.JContainer)JSon;
            bool GenerarTitulo = true;
            if (JSon.GetType().Name == "JArray")
            {
                foreach (Newtonsoft.Json.Linq.JObject Elemento in Contenedor)
                {
                    Newtonsoft.Json.Linq.JContainer ContenedorElemento = (Newtonsoft.Json.Linq.JContainer)Elemento;
                    if (GenerarTitulo)
                    {
                        string Titulos = "";
                        foreach (Newtonsoft.Json.Linq.JProperty Propiedad in ContenedorElemento)
                        {
                            Titulos = CeC.AgregaSeparador(Titulos, "\"" + Propiedad.Name + "\"", ",");
                        }
                        Csv = CeC.AgregaSeparador(Csv, Titulos, CeC.SaltoLinea);
                        GenerarTitulo = false;
                    }
                    string Linea = "";
                    foreach (Newtonsoft.Json.Linq.JProperty Propiedad in ContenedorElemento)
                    {
                        Linea = CeC.AgregaSeparador(Linea, "\"" + CeC.Convierte2String(((Newtonsoft.Json.Linq.JValue)(ContenedorElemento[Propiedad.Name])).Value) + "\"", ",");
                    }
                    Csv = CeC.AgregaSeparador(Csv, Linea, CeC.SaltoLinea);
                }
            }
            else
            {
        ...
        public static string Json2txt(string CadenaJson)
        {
            var JSon = Newtonsoft.Json.JsonConvert.DeserializeObject(CadenaJson);
            string txt = "";
            Newtonsoft.Json.Linq.JContainer Contenedor = (Newtonsoft.Json.Linq.JContainer)JSon;
            ...
            if (JSon.GetType().Name == "JArray")
            {
                foreach (Newtonsoft.Json.Linq.JObject Elemento in Contenedor)
                {
                    ...
                    if (GenerarTitulo)
                    {
                        ...
                        txt = CeC.AgregaSeparador(txt, Titulos, CeC.SaltoLinea);
                        GenerarTitulo = false;
                    }
                    string Linea = "";
                    foreach (Newtonsoft.Json.Linq.JProperty Propiedad in ContenedorElemento)
                    ...
                    txt = CeC.AgregaSeparador(txt, Linea, CeC.SaltoLinea);
                }
            }
            else
            {
        ...
        public static decimal ObtenValor(TimeSpan ST, int TipoUnidadID, int TipoRedondeoID)
        {
            ...
            return eClockBase.CeC.Convierte2Decimal(R);
        }
        ...
        public static bool Compara(object Objeto1, object Objeto2)
        {
            ...
            {
                double dObjeto1 = eClockBase.CeC.Convierte2Double(Objeto1);
                double dObjeto2 = eClockBase.CeC.Convierte2Double(Objeto2);
        ...
        public static string Convierte2DateWeb(object Fecha)
        {
            DateTime DT = Convierte2DateTime(Fecha);
            return DT.ToString("MM/dd/yyyy");
        }

        public static string ParchaJson(string Json)
        {
            if (Json == null || Json == "")
                return "";

            return Json.Replace("{\"@xml:space\":\"preserve\",\"#significant-whitespace\":\" \"}", "\" \"");
        }

        public static byte[] ImagenIgual
        {
            get
            {
                byte[] Img = new byte[1];
        ...
        public static bool EsImagenIgual(byte[] Imagen)
        {
            if (Imagen != null && Imagen.Length == 1 && Imagen[0] == (byte)'=')
                return true;
            return false;
        }
    }
