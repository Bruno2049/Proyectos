...
namespace eClock5.Clases
{
    class CeC_Registro
    {
        ...
        public static string ObtenIDWindows()
        {
            IntPtr key;
            int error;
            if ((error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, @"Software\Microsoft\Windows NT\CurrentVersion", 0, KEY_QUERY_VALUE | KEY_WOW64_64KEY, out key)) == 0)
            {
                try
                {
                    return RegQueryValue(key, "ProductId").ToString();
