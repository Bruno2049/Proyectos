...
namespace eClock5.Clases
{
    class Parametros
    {
        ...
        public static bool MuestraControl(Grid GridContenedor, UserControl Control, Parametros Param, bool DescargaAutomaticamente = true)
        {
            //GridContenedor.Children.Clear();
            Control.Tag = Param;
            GridContenedor.Children.Add(Control);
            if (DescargaAutomaticamente)
            {
                Control.IsVisibleChanged += delegate(object sender, DependencyPropertyChangedEventArgs e)
                    {
                        GridContenedor.Children.Remove(Control);
                    };
            }
            if (!Control.IsLoaded)
