...
function showError(error) {
    ...
    switch (error.code) {
        case error.PERMISSION_DENIED:
            errorVal = "Error: El usuario no permitió la toma de la geolocalización.";
            break;
        case error.POSITION_UNAVAILABLE:
            errorVal = "Error: No se pudo obtener la posición geográfica.";
            break;
        case error.TIMEOUT:
            errorVal = "Error: Se venció el tiempo de espera para obtener la posición geográfica.";
            break;
        case error.UNKNOWN_ERROR:
            errorVal = "Error: Ocurrió un error desconocido al intentar obtener la posición geográfica.";
            break;
    }
...
function GuardarParcialmente() {
    ...
    arrValoresGuardar["ExternalId"] = window.ordenActual.toString();
    arrValoresGuardar["AssignedTo"] = AssignedTo;
    arrValoresGuardar["FormiikResponseSource"] = "CapturaWeb" + (EsCallCenter.toUpperCase() == "TRUE" ? "CC" : "");

    
    // ReSharper disable once Html.IdNotResolved
    var pos = document.getElementById("PositionGps");
    if (pos != null && pos.value.substring(0, 4) == "Lat:") {
        if (!(Ruta.toUpperCase().indexOf("MYO") > -1)) {
            InicializarArrCobranza(pos);
        }
        ...
        if (Ruta.toUpperCase().indexOf("MYO") > -1) {
            ...
            if ($("[id^='ResultUnsuitable']").parent().not(".CWHidden").length + $("[id^='ResultNotWorkingInJob']").parent().not(".CWHidden").length > 0) {
                arrValoresGuardar["DictamenInValidReferece"] = "Referencia Invalida";
                contDictamen++;
                ...
                $.each(arr, function (i, val) {
                    var element = "ResultValidReference_Ref" + val;
                    resultado += ($("[id^='" + element + "']").parent().not(".CWHidden").length == parseInt(($("[id^='" + element + "']").parent().length) )) ? 1 : 0;
                });
                if (arr.length == resultado) {
                    arrValoresGuardar["DictamenValidReferece"] = "Referencia Valida";
                    contDictamen++;
                } else {
                    arrValoresGuardar["DictamenReapplyReferences"] = "Volver a solicitar referencias";
                    contDictamen++;
                }
                
            }

            $($("[id^='Result']").parent().not(".CWHidden,.RAnt")).children().each(function (i, val) {
                if ($(val).attr("id")!=undefined)
                    arrValoresGuardar[$(val).attr("id")] = $(val).val();
            });
        }
        //
        ...
        for (var k in arrValoresGuardar) {
            ...
            if (arrValoresGuardar.hasOwnProperty(k)) {
                ...
                if (arrValoresGuardar[k]==undefined) {
                    alert("No se puede guardar la gestión por que el valor del campo " + k + " es nulo. Si el problema continua reportelo a soporte.");
                    guardarOk = false;
                    break;
                }
...
function GuardarValor(inputActual) {
    ...
    var obj = inputActual;
    if (obj.className.indexOf("CWDisabled") < 0 || obj.className.indexOf("CWVariables") > 0) {
        if (obj.parentElement.className.indexOf("CWHidden") < 0 || obj.parentElement.className.indexOf("CWVariables") > 0) {
            switch (obj.type) {
                case "radio":
                    var valRadioSel = $("input[name='" + obj.name + "']:checked").val();
                    if (valRadioSel != undefined) {
...
function ValC(campo, valor,comparacion) {
    var valBuscadoT = arrValores[campo.toUpperCase()];
    if (valBuscadoT != undefined) 
        {

            var valBuscado = valBuscadoT.toUpperCase();
            var valorBuscar = valor.toUpperCase();

            if ($.isNumeric(valBuscadoT) || $.isNumeric(valorBuscar)) {
                valBuscado = parseFloat(valBuscadoT);
                valorBuscar = parseFloat(valorBuscar);
            }
        

            switch (comparacion) {
                case "==":
                    return valBuscado == valorBuscar;
                case ">=":
                    return valBuscado >= valorBuscar;
                case ">":
                    return valBuscado > valorBuscar;
...
function ComplementoXDictamen(dictamen) {
    ...
    else if (dictamen.toUpperCase().indexOf("DCP") > 0 && !EsCallCenter.toUpperCase() == "TRUE") {
