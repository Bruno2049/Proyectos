...
-- =============================================
CREATE PROCEDURE [dbo].[ObtenerIndDashRDST_CREDSOLUCIONADOS] @idUsuario INT, @delegacion VARCHAR(10), @despacho VARCHAR(10), @supervisor VARCHAR(10), @gestor VARCHAR(10), @tipoFormulario VARCHAR(50), @contraPorcentaje INT, @callCenter VARCHAR(6) = 'false'
		...
		BEGIN
			...
			WHERE o.Estatus IN (3,4)
				...
				AND o.idDictamen IN (
					...
					WHERE Nombre IN ('DictamenFPP', 'Dictamenpromdepago', 'DictamenpromdepagoTOM', 'DictamenFPPTOM', 'DictamenFPPMen') 
					AND CV_RUTA = @ruta
					)
