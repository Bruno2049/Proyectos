...
-- =============================================
ALTER PROCEDURE [dbo].[ObtenerIndDashDASH_CREDVISITADOS]
	@idUsuario  int,
...
SELECT @parte=fi_Parte,@nombreDisplay=fc_Descripcion FROM dbo.Utils_Descripciones with (nolock) WHERE fc_Clave = 'DASH_CREDVISITADOS' 
...
BEGIN	
	...
	where o.Estatus in(3,4,6) AND o.cvRuta=@ruta  AND o.idUsuario <> 0 AND o.idDominio>1 
...
BEGIN
	...
	BEGIN	
		...
		from (select CV_CREDITO,CV_Ruta,CV_DELEGACION,CC_DESPACHO from Creditos with (nolock)) c 
		left join (select Estatus,idOrden,idUsuario,num_Cred from Ordenes with (nolock)) o on o.num_Cred=c.CV_CREDITO 
		left join (select nom_corto,idDominio from Dominio with (nolock)) d on d.nom_corto=c.CC_DESPACHO 
		where CV_Ruta=@ruta and o.Estatus in(3,4,6) and d.idDominio>1 AND o.idUsuario <> 0			
