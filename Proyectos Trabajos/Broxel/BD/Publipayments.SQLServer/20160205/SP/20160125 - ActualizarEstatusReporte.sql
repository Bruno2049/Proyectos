...
/****** Object:  StoredProcedure [dbo].[ActualizarEstatusReporte]    Script Date: 15/02/2016 04:12:18 p.m. ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[ActualizarEstatusReporte] (@idUsuarioReporte INT, @TipoReporte INT)
	...
	ELSE IF(@TipoReporte=2)
		...
		BEGIN
			...
			VALUES (
				...
				SELECT SCOPE_IDENTITY()Resultado
				RETURN
		END
		ELSE
			BEGIN
				...
				SELECT idreporte AS Resultado FROM  [dbo].[Reportes]
				WHERE Tipo = @TipoReporte
					AND idPadre = @idUsuarioReporte
					AND CONVERT(CHAR(7), Fecha, 120) = CONVERT(CHAR(7), GETDATE(), 120) 
