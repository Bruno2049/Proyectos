/****** Object:  StoredProcedure [dbo].[ObtenerIndDashRDST_GESTPROMDIA]    Script Date: 08/12/2016 12:09:06 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[ObtenerIndDashRDST_GESTPROMDIA]
	@idUsuario  int,
...
DECLARE 
	...
	SELECT @diastranscurridos = day(dateadd (dd, datediff ( dd, - 1, GETDATE()), - 1))

	SELECT @ruta = Ruta FROM Formulario WITH (NOLOCK) WHERE idFormulario = @tipoFormulario

	SELECT @parte = fi_Parte, @nombreDisplay = fc_Descripcion FROM dbo.Utils_Descripciones WITH (NOLOCK) WHERE fc_Clave = 'RDST_GESTPROMDIA' 
	...
	BEGIN
		...
		BEGIN	
			...
			AND d.idDominio = CASE  WHEN @despacho = '%' THEN d.idDominio ELSE @despacho END 
				...
				SELECT @valor = CAST ((@valorGestiones / @diastranscurridos) AS VARCHAR (10))
