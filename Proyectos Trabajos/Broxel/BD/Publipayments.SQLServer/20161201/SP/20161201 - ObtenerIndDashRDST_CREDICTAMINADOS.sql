/****** Object:  StoredProcedure [dbo].[ObtenerIndDashRDST_CREDICTAMINADOS]    Script Date: 30/11/2016 06:07:38 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[ObtenerIndDashRDST_CREDICTAMINADOS] @idUsuario INT, @delegacion VARCHAR(10), @despacho VARCHAR(10), @supervisor VARCHAR(10), @gestor VARCHAR(10), @tipoFormulario VARCHAR(50), @contraPorcentaje INT, @callCenter VARCHAR(6) = 'false'
...
BEGIN
	...
	WHERE fc_Clave = 'RDST_CREDICTAMINADOS'
		...
		BEGIN
			...
			WHERE o.Estatus IN (3, 4)
				...
				AND o.idDictamen IN (
					...
					WHERE Nombre NOT IN ('DictamenPromesaPago','DictamenPPFPP','DictamenLiquidacion','DictamenCONFPRParcial','DictamenProrrogaP','DictamenFPP') 
					AND CV_RUTA = @ruta
					)
