...
/****** Object:  StoredProcedure [dbo].[FuncionesJsComplemento]    Script Date: 12/10/2016 12:24:49 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	-- Cambios en textos para callcenter, solo aplica si el formulario tiene nombre de callcenter 
		...
		IF((select 1 from formulario where idformulario=@idformulario and nombre like '%Callcenter%')=1)
			...
			INSERT  INTO [dbo].[FuncionesXCampos] (idFuncionJS,idCampoFormulario) SELECT @idFuncionJs,idCampoFormulario from CamposXSubFormulario where NombreCampo='CalificaProrroga' and idSubFormulario in (select idSubFormulario from SubFormulario where idFormulario=@idformulario) 
