	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT CAST(ROW_NUMBER() OVER (
							ORDER BY tablaResultados.valor DESC, tablaSupervisores.Description
							) AS INT) AS Posicion, tablaSupervisores.Value AS Identificador, tablaSupervisores.Description AS Usuario, tablaSupervisores.Nombre AS Nombre, COALESCE(tablaResultados.valor, 0) AS Valor, CASE 
						WHEN @valorSuperior = 0
							THEN '0'
						ELSE (COALESCE(tablaResultados.valor, 0) * 100) / @valorSuperior
						END AS Porcentaje
