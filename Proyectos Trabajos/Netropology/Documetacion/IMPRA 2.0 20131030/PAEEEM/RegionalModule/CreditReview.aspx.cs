            ...
            if (Request["statusid"] != null)
            {
                ...
                if (statusid > 0)
                {
                        ...
                        statusid != (int)PAEEEM.Helpers.CreditStatus.TARIFA_FUERA_DE_PROGRAMA && statusid != (int)PAEEEM.Helpers.CreditStatus.Calificación_MOP_no_válida)
                    {
                        ...
                        btnCreditHistroyReview.Visible = true;
                        // End
                    }
                    ...
                    //    btnCreditHistroyReview.Visible = false;
                    //}
                    // Add by Tina 2011/08/09
                    else
                    {
...
//                    }
        ...
        private void InitDepartmentProperty()
        {
            if (Session["UserInfo"] != null)
            {
                ...
                else if (User.Tipo_Usuario == GlobalVar.REGIONAL_OFFICE)
                {
                    if (Request["creditno"] != null)
                    {
        ...
        private void LoadCreditFromData(DataTable dtCredit)
        {
            ...
            // RSA 20130813 Enable editing
            DataTable dtCat_Auxilira = CAT_AUXILIARDal.ClassInstance.Get_CAT_AUXILIARByCreditNo(dtCredit.Rows[0]["No_Credito"].ToString());
                ...
                && CompanyType.REPECO != (CompanyType)Enum.Parse(typeof(CompanyType), ddlPfisica.SelectedItem.Value))
            {
                ...
                lblBirthDate.Text = HttpContext.GetGlobalResourceObject("DefaultResource", "LabelBirthDateMoral") as string;
                lblName.Text = HttpContext.GetGlobalResourceObject("DefaultResource", "LabelPersonaRazon") as string;
                if (dtCat_Auxilira.Rows.Count > 0)
                {
            ...
            else
            {
                if (dtCat_Auxilira.Rows.Count > 0)
                {
                    ...
                    txtName.Text = dtCat_Auxilira.Rows[0]["Dx_Nombres"].ToString();
                    txtLastname.Text = dtCat_Auxilira.Rows[0]["Dx_Apellido_Paterno"].ToString();
                    txtMotherName.Text = dtCat_Auxilira.Rows[0]["Dx_Apellido_Materno"].ToString();
                    txtBirthDate.Text = string.Format("{0:yyyy-MM-dd}",dtCat_Auxilira.Rows[0]["Dt_Nacimiento_Fecha"]);//Changed by Jerry 2011/08/09
            ...
            //ddlAcreditado 1
            //rblSex 1
            rblSex.SelectedValue = dtCredit.Rows[0]["FG_SEXO_REPRE_LEGAL"].ToString();
            txtRPU.Text = dtCredit.Rows[0]["NO_RPU"].ToString();
            //rblEstadoCivil 1
            ...
            txtPromedioMV.Text = dtCredit.Rows[0]["MT_VENTAS_MES_EMPRESA"].ToString();
            txtTotalGastosMensual.Text = dtCredit.Rows[0]["MT_GASTOS_MES_EMPRESA"].ToString();
            ...
            this.txtTotalCost.Text = K_CREDITO_COSTODal.ClassInstance.GetTotalCost(this.Creditno);
            this.txtTotalDescount.Text = K_CREDITO_DESCUENTODal.ClassInstance.GetTotalDescount(this.Creditno);            
            ...
            TotalCost = string.IsNullOrEmpty(this.txtTotalCost.Text)?default(decimal): decimal.Parse(this.txtTotalCost.Text);
            TotalDiscount = string.IsNullOrEmpty(this.txtTotalDescount.Text) ? default(decimal) : decimal.Parse(this.txtTotalDescount.Text);
            ...
            SubTotal = (SubTotal + Gastos) / (1 + Convert.ToDecimal(pctIVA) / 100);
        ...
        private void LoadDDLData(DataTable dtCredit)
        {
           ...
           DataTable Industria = CAT_TIPO_INDUSTRIADal.ClassInstance.Get_All_CAT_TIPO_INDUSTRIA();
            if ( Industria != null && Industria.Rows.Count > 0 )
            {
                ...
                ddlGiroEmpresa.DataTextField = "Dx_Tipo_Industria";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //end add
                CreditEntity creditEntity = GetDataFromUI();
                CAT_AUXILIAREntity catAuxiliar = new CAT_AUXILIAREntity();

                creditEntity.No_Credito = Creditno;
                ...
                if (StatusFlag.Equals("P"))
                {
                    ...
                    {
                        #region "DATOS DEL CO-ACREDITADO"
                        creditEntity.Dx_Nombre_Coacreditado = txtNobreCoAcreditadoRS.Text;
                        creditEntity.Dx_RFC_CURP_Coacreditado = txtCoAcreditadoCURP.Text;
                        int sex10 = 1;
                        int.TryParse(rblCoAcreditadoSexo.SelectedValue, out sex10);
                        creditEntity.Fg_Sexo_Coacreditado = sex10;

                        creditEntity.Dx_Telefono_Coacreditado = txtCoAcreditadoTele.Text;
                        creditEntity.Dx_Domicilio_Coacreditado_Calle = txtCoAcreditadoCalle.Text;
                        creditEntity.Dx_Domicilio_Coacreditado_Num = txtCoAcreditadoNumero.Text;
                        creditEntity.Dx_Domicilio_Coacreditado_CP = txtCoAcreditadoCP.Text;
                        creditEntity.Dx_Domicilio_Coacreditado_Colonia = txtDx_Domicilio_Coacreditado_Colonia.Text;
                        int coacreestado = 0;
                        int.TryParse(ddlCoAcreditadoEstado.SelectedValue, out coacreestado);
                        creditEntity.Cve_Estado_Coacreditado = coacreestado;
                        int coacredm = 0;
                        int.TryParse(ddlCoAcreditadoDM.SelectedValue, out coacredm);
                        creditEntity.Cve_Deleg_Municipio_Coacreditado = coacredm;
                        #endregion
                    }
                    ...
                    creditEntity.Mt_Gastos_Instalacion_Mano_Obra = (double)gastosTotal;

                    if (!string.IsNullOrEmpty(txtNo_Plazo_Pago.Text))
                    {
                        creditEntity.No_Plazo_Pago = Convert.ToInt32(txtNo_Plazo_Pago.Text);
                    }

                    if (!string.IsNullOrEmpty(ddlDx_periodo_pago.SelectedValue))
                    {
                        creditEntity.Cve_Periodo_Pago = Convert.ToInt32(ddlDx_periodo_pago.SelectedValue);
                    }
                    //end add

                    #region Calculation Fields
                    //Mt_Monto_Solicitado
                    creditEntity.Mt_Monto_Solicitado = (!string.IsNullOrEmpty(this.txtTotal.Text)) ? Convert.ToDouble(this.txtTotal.Text) : default(double);
                    //No_Ahorro_Energetico
                    double dbEffciencyProductSelect = ProductCapacity;//Changed by coco 2011-08-04

                    if (Dics["HighAvgConsumption"] != "")
                    {
                        creditEntity.No_Ahorro_Energetico = Convert.ToDouble(System.Math.Abs((double.Parse(Dics["HighAvgConsumption"]) - dbEffciencyProductSelect / 12)));
                    }
                    if (Dics["HighAvgConsumption"] != "")
                    {
                        creditEntity.No_consumo_promedio = float.Parse(Dics["HighAvgConsumption"]);//;                   
                    }

                    //No_Ahorro_Economico
                    double iAverageConsume = 0;
                    iAverageConsume = (double)creditEntity.No_consumo_promedio;
                    double originalConsumption = 0;
                    creditEntity.No_Ahorro_Economico = K_CREDITOBll.ClassInstance.CalculateEconomicConsumptionSavings(creditEntity.Cve_Estado_Neg, iAverageConsume, creditEntity.Mt_Monto_Solicitado, strPct_Tasa_IVA / 100, Dics["Rate"].ToString(), ref originalConsumption);                    //Mt_Capacidad_Pago

                    //Mt_Capacidad_Pago
                    creditEntity.Mt_Capacidad_Pago = K_CREDITOBll.ClassInstance.CalculatePaymentCapacity(creditEntity.No_Plazo_Pago, creditEntity.Mt_Ingreso_Neto_Mes_Empresa, creditEntity.No_Ahorro_Economico, Pct_CAT / 100, creditEntity.Cve_Periodo_Pago);
                    #endregion

                    creditEntity.Cve_Estatus_Credito = (int)CreditStatus.PENDIENTE;
                    //K_CREDITO_AMORTIZACION
                    DataTable CreditAmortizacionDt = K_CREDITOBll.ClassInstance.CalculateCreditAmortizacion(Creditno, creditEntity.Mt_Monto_Solicitado, Pct_Tasa_Fija / 100, creditEntity.No_Plazo_Pago, creditEntity.Cve_Periodo_Pago, Pct_Tasa_Interes / 100, strPct_Tasa_IVA / 100, Pct_CAT / 100, Dics["PeriodEndDate"]);
                    if (CreditAmortizacionDt != null)
                    {
                        ...
                        creditEntity.Mt_Monto_Total_Pagar = deTemp;

                        if (creditEntity.Mt_Monto_Solicitado > creditEntity.Mt_Capacidad_Pago)
                        {
                        ...
                        {
                            int result = K_CREDITOBll.ClassInstance.Update_CreditReview(creditEntity, CreditProduct, CreditAmortizacionDt, catAuxiliar, creditCostEntity, creditDesEntity);
                ...
                else
                {
                    ...
                    {
                        creditEntity.Cve_Estatus_Credito = (int)CreditStatus.BENEFICIARIO_CON_ADEUDOS;
                        creditEntity.Dt_Fecha_Beneficiario_con_adeudos = DateTime.Now.Date;
                    }
                    else if (StatusFlag == "A")//Tarifa fuera de programa status
                    {
                        creditEntity.Cve_Estatus_Credito = (int)CreditStatus.TARIFA_FUERA_DE_PROGRAMA;
                        creditEntity.Dt_Fecha_Tarifa_fuera_de_programa = DateTime.Now.Date;
                    }

                    int result = K_CREDITOBll.ClassInstance.Update_CreditReviewExceptProduct(creditEntity, catAuxiliar);
                ...
                StatusID = creditEntity.Cve_Estatus_Credito;
        ...
        // Add by Tina 2011/08/09
        protected void btnCreditHistroyReview_Click(object sender, EventArgs e)
        {
            if (StatusID == (int)PAEEEM.Helpers.CreditStatus.PENDIENTE)
            {
                Response.Redirect("../SupplierModule/ValidateCrediticialHistroy.aspx?CreditNumber=" + Convert.ToBase64String(System.Text.Encoding.Default.GetBytes(Creditno.ToString())).Replace("+", "%2B") + " &Flag=M");
            }
            else
            {
                string Flag = Request.QueryString["Flag"] != null ? Request.QueryString["Flag"] : "";
                Response.Redirect("../SupplierModule/CrediticialHistroyReview.aspx?CreditNumber=" + Convert.ToBase64String(System.Text.Encoding.Default.GetBytes(Creditno.ToString())).Replace("+", "%2B") + " &Flag=" + Flag);
            }
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        private CreditEntity GetDataFromUI()
        {
            CreditEntity creditEntity = new CreditEntity();

            creditEntity.Dx_Razon_Social = txtName.Text + " " + txtLastname.Text + " " + txtMotherName.Text;
            int industryid = 0;
            int.TryParse(ddlGiroEmpresa.SelectedValue, out industryid);
            creditEntity.Cve_Tipo_Industria = industryid;
            int pfisica = 0;
            int.TryParse(ddlPfisica.SelectedValue, out pfisica);
            creditEntity.Cve_Tipo_Sociedad = pfisica;
            creditEntity.Dx_CURP = txtCURP.Text;
            creditEntity.Dx_RFC = txtRFC.Text;
            creditEntity.Dx_Nombre_Repre_Legal = txtRepresLegal.Text;
            int acreditado = 0;
            int.TryParse(ddlAcreditado.SelectedValue, out acreditado);
            creditEntity.Cve_Acreditacion_Repre_legal = acreditado;
            int sex03 = 0;
            int.TryParse(rblSex.SelectedValue, out sex03);
            creditEntity.Fg_Sexo_Repre_legal = sex03;
            creditEntity.No_RPU = txtRPU.Text;
            int estadocivil = 0;
            int.TryParse(rblEstadoCivil.SelectedValue, out estadocivil);
            creditEntity.Fg_Edo_Civil_Repre_legal = estadocivil;

            int regimenmatri = 0;
            int.TryParse(ddlRegimenMatri.SelectedValue, out regimenmatri);
            creditEntity.Cve_Reg_Conyugal_Repre_legal = regimenmatri;
            int tipoindenti = 0;
            int.TryParse(ddlTipoIdenti.SelectedValue, out tipoindenti);
            creditEntity.Cve_Identificacion_Repre_legal = tipoindenti;
            creditEntity.Dx_No_Identificacion_Repre_Legal = txtNumero.Text;
            double promediomv = 0;
            double.TryParse(txtPromedioMV.Text, out promediomv);
            creditEntity.Mt_Ventas_Mes_Empresa = promediomv;
            double totalgasm = 0;
            double.TryParse(txtTotalGastosMensual.Text, out totalgasm);
            creditEntity.Mt_Gastos_Mes_Empresa = totalgasm;
            creditEntity.Mt_Ingreso_Neto_Mes_Empresa = creditEntity.Mt_Ventas_Mes_Empresa - creditEntity.Mt_Gastos_Mes_Empresa;

            creditEntity.Dx_Email_Repre_legal = txtRLemail.Text;
            creditEntity.Dx_Domicilio_Fisc_Calle = txtFiscalCalle.Text;
            creditEntity.Dx_Domicilio_Fisc_Num = txtFiscalNumero.Text;
            creditEntity.Dx_Domicilio_Fisc_CP = txtFiscalCP.Text;
            creditEntity.Dx_Domicilio_Fisc_Colonia = txtDx_Domicilio_Fisc_Colonia.Text;
            int fiscalestado = 0;
            int.TryParse(ddlFiscalEstado.SelectedValue, out fiscalestado);
            creditEntity.Cve_Estado_Fisc = fiscalestado;
            int fiscaldm = 0;
            int.TryParse(ddlFiscalDM.SelectedValue, out fiscaldm);
            creditEntity.Cve_Deleg_Municipio_Fisc = fiscaldm;
            int fiscaltipopropie = 0;
            int.TryParse(ddlFiscalTipoPropie.SelectedValue, out fiscaltipopropie);
            creditEntity.Cve_Tipo_Propiedad_Fisc = fiscaltipopropie;
            creditEntity.Dx_Tel_Fisc = txtFiscalTele.Text;
            bool fgmdnego = true;
            bool.TryParse(cbMismoFiscal.Text, out fgmdnego);
            creditEntity.Fg_Mismo_Domicilio = fgmdnego;

            creditEntity.Dx_Domicilio_Neg_Calle = txtNegocioCalle.Text;
            creditEntity.Dx_Domicilio_Neg_Num = txtNegocioNumero.Text;
            creditEntity.Dx_Domicilio_Neg_CP = txtNegocioCP.Text;
            creditEntity.Dx_Domicilio_Neg_Colonia = txtDx_Domicilio_Neg_Colonia.Text;
            int negoestado = 0;
            int.TryParse(ddlNegocioEstado.SelectedValue, out negoestado);
            creditEntity.Cve_Estado_Neg = negoestado;
            int negdm = 0;
            int.TryParse(ddlNegocioDM.SelectedValue, out negdm);
            creditEntity.Cve_Deleg_Municipio_Neg = negdm;
            int negotipopropie = 0;
            int.TryParse(ddlNegocioTipoPropie.SelectedValue, out negotipopropie);
            creditEntity.Cve_Tipo_Propiedad_Neg = negotipopropie;
            creditEntity.Dx_Tel_Neg = txtNegocioTele.Text;
            creditEntity.Dx_Nombre_Aval = txtNobreAvalRS.Text;
            creditEntity.Dx_RFC_CURP_Aval = txtAvalCURP.Text;
            creditEntity.Dx_Tel_Aval = txtAvalTele.Text;
            //creditEntity.No_RPU_AVAL = this.txtNo_RPU_aval.Text;
            int sex09 = 1;
            int.TryParse(rblAvalSexo.SelectedValue, out sex09);
            creditEntity.Fg_Sexo_Aval = sex09;
            creditEntity.Dx_Domicilio_Aval_Calle = txtAvalCalle.Text;
            creditEntity.Dx_Domicilio_Aval_Num = txtAvalNumero.Text;
            creditEntity.Dx_Domicilio_Aval_CP = txtAvalCP.Text;
            creditEntity.Dx_Domicilio_Aval_Colonia = txtDx_Domicilio_Aval_Colonia.Text;
            int avalestado = 0;
            int.TryParse(ddlAvalEstado.SelectedValue, out avalestado);
            creditEntity.Cve_Estado_Aval = avalestado;
            int avaldm = 0;
            int.TryParse(ddlAvalDM.SelectedValue, out avaldm);
            creditEntity.Cve_Deleg_Municipio_Aval = avaldm;
            double avalpmv = 0;
            //double.TryParse(txtAvalPMV.Text, out avalpmv);
            creditEntity.Mt_Ventas_Mes_Aval = avalpmv;
            double avaltgm = 0;
            //double.TryParse(txtAvalTGM.Text, out avaltgm);
            creditEntity.Mt_Gastos_Mes_Aval = avaltgm;
            creditEntity.Mt_Ingreso_Neto_Mes_Aval = creditEntity.Mt_Ventas_Mes_Aval - creditEntity.Mt_Gastos_Mes_Aval;
            ...
            for (int i = 0; i < GridViewDataSource.Rows.Count; i++)
            {
                ...
                if (ddlCapacidad != null)
                {
                    ...
                    if (Capacity != null)
                    {
                        ...
                        ddlCapacidad.DataTextField = "Ft_Capacidad";
                        ddlCapacidad.DataValueField = "Cve_Producto_Capacidad";
