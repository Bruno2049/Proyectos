...
namespace PAEEEM.DataAccessLayer
{
    ...
    public class CAT_PRODUCTODal
    {
        ...
        public int ObtienePrecioDistribuidor(int Cve_Modelo, int Id_Proveedor)
        {
            int result = 0;
            try
            {
                string SQL = "select mt_precio_unitario from K_PROVEEDOR_PRODUCTO where Cve_Producto=" + Cve_Modelo.ToString() +
                    " and Id_Proveedor=" + Id_Proveedor.ToString();
        ...
        public DataTable Get_CAT_PRODUCTO_ByTechnology_1(string strTechnology, string strProvider, string strTypeProduct)
        {
            try
            {
                ...
                SQL = SQL + " left join K_PROVEEDOR_PRODUCTO D on D.Cve_Producto=A.Cve_Producto and D.Id_Proveedor=@Id_Proveedor ";
        ...
        public DataTable GetModelByTechnologyAndMarcaAndProveedor(string technology, string marca, string IdProveedor)
        {
            try
            {
                string SQL = "select distinct Dx_Modelo_Producto from CAT_PRODUCTO where 1=1 and Cve_Estatus_Producto<>@Cve_Estatus_Producto";
                ...
                SQL = SQL + " AND Cve_Producto in (Select Cve_Producto FROM K_PROVEEDOR_PRODUCTO WHERE id_proveedor=" + IdProveedor + ")";
                SqlParameter[] paras = new SqlParameter[] { 
                    new SqlParameter("@Cve_Tecnologia", technology),
                    new SqlParameter("@Cve_Marca",marca),
                    new SqlParameter("@Cve_Estatus_Producto",ProductStatus.CANCELADO)
