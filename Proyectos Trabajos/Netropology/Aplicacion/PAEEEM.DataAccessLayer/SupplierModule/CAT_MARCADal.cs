...
namespace PAEEEM.DataAccessLayer
{
    ...
    public class CAT_MARCADal
    {
        ...
        public DataTable Get_CAT_MARCAByIdProveedorDal(string strProduct, string idProveedor)
        {
            try
            {
                string SQL = " select distinct A.* from CAT_MARCA  A " +
                    "inner join CAT_PRODUCTO B on A.Cve_Marca=B.Cve_Marca and B.Cve_Producto in (" + strProduct + ") " +
                    " and B.cve_producto in (select cve_producto from K_PROVEEDOR_PRODUCTO WHERE Id_Proveedor = " + idProveedor + ")";
        ...
        public DataTable Get_CAT_MARCAByIdProveedor(string strProduct, string idProveedor)
        {
            try
            {
                string SQL = " select distinct A.* from CAT_MARCA  A " +
                    "inner join CAT_PRODUCTO B on A.Cve_Marca=B.Cve_Marca and B.Cve_Tecnologia in (" + strProduct + ") " +
                    " and B.cve_producto in (select cve_producto from K_PROVEEDOR_PRODUCTO WHERE Id_Proveedor = " + idProveedor + ")";
