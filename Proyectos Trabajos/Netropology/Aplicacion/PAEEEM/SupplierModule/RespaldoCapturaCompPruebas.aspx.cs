...
{
    public partial class RespaldoCapturaCompPruebas : Page
    {
                        ...
                        if (wuAltaBajaEquipos1.ValidaEquiposAltayBaja())
                        {
                                ...
                                resultado.NuevaFacturaNegativaValue && resultado.PsrValue && resultado.ValidacionValue)
                            {
                                if (wuAltaBajaEquipos1.ValidaMontoMaximo())
                                {
                                    ...
                                    if (wuAltaBajaEquipos1.CrearClienteONegocio(IdProveedor, IdSucursal, IdNegocio, cliente))
                                    {
        ...
        protected void CargaDatosPymeCaptura(CLI_Cliente cliCliente)
        {
            ...
            if (datosPyme != null)
            {
                DDXGiroEmpresa.SelectedValue = datosPyme.Cve_Tipo_Industria.ToString();
                DDXSector.SelectedValue = datosPyme.Cve_Sector_Economico.ToString();
                TxtNombreComercial.Text = datosPyme.Dx_Nombre_Comercial;
                TxtNoEmpleados.Text= datosPyme.No_Empleados.ToString();
                TxtGastosMensuales.Text = datosPyme.Tot_Gastos_Mensuales.ToString();
                TxtVentasAnuales.Text = datosPyme.Prom_Vtas_Mensuales.ToString();                
                TxtCP.Text = datosPyme.Codigo_Postal;
                
                DDXEstado.SelectedValue = datosPyme.Cve_Estado.ToString();
                LlenarDDxMunicipo((int)datosPyme.Cve_Estado, DDXMunicipio, DDXColonia);
                DDXMunicipio.SelectedValue = datosPyme.Cve_Deleg_Municipio.ToString();
                LlenarDDxColonias((int)datosPyme.Cve_Estado, (int)datosPyme.Cve_Deleg_Municipio, DDXColonia, DDXColoniaHidden);
                DDXColonia.SelectedValue = datosPyme.Cve_CP.ToString();

                DDXTipoPersona.SelectedValue = datosPyme.Cve_Tipo_Sociedad.ToString();
                ...
                if (datosPyme.Cve_Tipo_Sociedad == 1)
                {
                    ...
                    TxtFechaNacimientoVPyME.Text = fechaNac.Year.ToString() + "-" +
                                                   fechaNac.Month.ToString().PadLeft(2, '0') +
                                                   "-" + fechaNac.Day.ToString().PadLeft(2, '0');

                    TxtRFCFisicaVPyME.Text = datosPyme.RFC;
                    divRFCFisica.Visible = true;
                    divRFCMoral.Visible = false;
                }
                else
                {
                    ...
                    TxtRFCMoralVPyME.Text = datosPyme.RFC;
                    TxtFechaConstitucionVPyME.Text = fechaCons.Year.ToString() + "-" +
                                                   fechaCons.Month.ToString().PadLeft(2, '0') +
                ...
                if (esPyme)
                {
                    ...
                    if (validaDirecciones)
                    {
                        ...
                        if (newDatosPyme.Cve_CP != domicilioNegocio.CVE_CP)
                        {
                            ...
                            if (domicilioNegocio.CVE_CP == domicilioFiscal.CVE_CP)
                            {
        ...
        protected void CargaDatosCliente()
        {
            ...
            if (cliente.DatosCliente.Cve_Tipo_Sociedad == 1)
            {
                ...
                LlenarDDxSexoCBas();
                if (cliente.DatosCliente.Genero != null)
                {
                ...
                DDXRegimenMatrimonialCBas.SelectedValue = cliente.DatosCliente.IdRegimenConyugal == null
                    ? "Seleccione"
            ...
            if (cliente.DatosCliente.Cve_Tipo_Sociedad == 2)
            {
                ...
                TxtFechaConstitucionCBas.Text = fechaCons.Year.ToString() + "-" + fechaCons.Month.ToString().PadLeft(2, '0') +
                                           "-" + fechaCons.Day.ToString().PadLeft(2, '0');
                TxtRFCMoralCBas.Text = cliente.DatosCliente.RFC;
                if (negocio.IdTipoIdentificacion != null)
                {
       ...
       // }
        ...
        public void CargaDatosCapturaBasica()
        {
          ...
          //LlenarCatalogoEstadosCBas();
            ...
            if (datosPyme != null)
            {
                ...
                {
                    TxtRFCMoralCBas.Text = datosPyme.RFC;
        ...
        public Cliente ObtenCliente()
        {
            ...
            if (DDXTipoPersona.SelectedValue == "2")
            {
                ...
                cliente.DatosCliente.RFC = TxtRFCMoralCBas.Text;
        ...
        public Cliente GetObjetoCliente()
        {
           var getDatosCliente = new Cliente
            ...
            direcciones = new DIR_Direcciones
            {
                ...
                Cve_Tipo_Propiedad = Convert.ToInt16(DDXTipoPropiedadFiscalCBas.SelectedValue),
       ...
       #region Metodos Protegidos
        ...
        public bool GuardaInfoCompCliente()
        {
            ...
            var datosInfoCompCliente = SolicitudCreditoAcciones.ActualizaInfoComplementaria(cliente, Convert.ToInt16(Session["IdUserLogueado"]),
        ...
        protected void BtnImgEditar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (dataKey != null)
            {
                ...
                if (existe != null)
                {
                    ...
                    txtModelo.Text = existe.Dx_Marca == null && existe.Dx_Color == null
                        ? string.Empty
                        : existe.Dx_Modelo_Producto;
                    txtAntiguedad.Value = existe.Dx_Antiguedad == null
                        ? (double?) null
                        : int.Parse(existe.Dx_Antiguedad);

                    // Si ya esta capturado se selecciona y se bloquea para que no se pueda modificar
                    if (existe.Id_Centro_Disp != null)
                    {
                        string TipoCentro = existe.Fg_Tipo_Centro_Disp.Equals("M") ? "-(Matriz)" : "-(Branch)";
        ...
        protected void CargaDatosBajaEquiposCComp()
        {
            ...
            var dt = InfoCompAltaBajaEquipos.Get_Info_Equipos_Baja(NumeroCredito);
            grdEquiposBaja.DataSource = dt;
            ...
            if (requiereCayd != null)
            {
                ...
                {
                    drpCAyD.SelectedValue = string.Format("{0}-({1})", CAyDAsignado.Id_Centro_Disp, CAyDAsignado.Fg_Tipo_Centro_Disp.Equals("M")?"Matriz":"Branch");
                }
            }
            else
            {
