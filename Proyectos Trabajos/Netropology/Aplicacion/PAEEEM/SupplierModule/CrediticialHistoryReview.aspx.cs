...
namespace PAEEEM
{
    public partial class CrediticialHistoryReview : Page
    {
        ...
        private void InitDefaultData()
        {
            ...
            if (dtCredito != null && dtCredito.Rows.Count > 0)
            {
                ...
                if (!Convert.IsDBNull(dtCredito.Rows[0]["Monto_Solicitado"]))
                {
                    this.txtRequestAmount.Text = string.Format("{0:C2}", dtCredito.Rows[0]["Monto_Solicitado"]);
                }
                ...
                if (!Convert.IsDBNull(dtCredito.Rows[0]["Dx_Periodo_Pago"]) &&
                    dtCredito.Rows[0]["Dx_Periodo_Pago"].ToString() != "")
                {
            ...
            try
            {
                var rn_aval = beRefNotariales.Obtener(int.Parse(cre.IdCliente.ToString()), 2);
                var rn_AdicAval = beRefNotariales.Obtener(int.Parse(cre.IdCliente.ToString()), 9);

                txtGuarantee_LegalDocumentNumber.Text =
                    rn_aval.Numero_Escritura.ToString();

                if (rn_aval.Fecha_Escritura != null &&
                    rn_aval.Fecha_Escritura.ToString() != "")
                {
                    txtGuarantee_LegalDocumentFecha.Text =
                        Convert.ToDateTime(rn_aval.Fecha_Escritura.ToString()).ToString("yyyy-MM-dd");
                }

                txtGuarantee_NotariesProfessionName.Text =
                    rn_aval.Nombre_Notario.ToString();
                txtGuarantee_NotariesProfessionNumber.Text =
                    rn_aval.Numero_Notaria.ToString();

                if (rn_aval.Estado != null)
                {
                    drpGuarantee_Estado.SelectedValue =
                        rn_aval.Estado.ToString();
                    GuaranteeDelegMunicipioBind(); // Filter Deleg_Municipio_Escritura_Aval by Estado_Escritura_Aval
                }

                if (rn_aval.Municipio != null)
                {
                    drpGuarantee_OfficeOrMunicipality.SelectedValue =
                        rn_aval.Municipio.ToString();
                }

                if (cre.Adquisicion_Sust == 2)
                {
                ...
                if (rn_AdicAval != null)
                {
                    if(rn_AdicAval.Fecha_Escritura != null)
                        txtPropertyEncumbrancesFecha.Text = Convert.ToDateTime(rn_AdicAval.Fecha_Escritura.ToString()).ToString("yyyy-MM-dd");

                    txtMarriageNumber.Text = rn_AdicAval.Numero_Escritura;
                    txtPropertyEncumbrancesName.Text = rn_AdicAval.Nombre_Notario;
                    txtCitizenRegisterOffice.Text = rn_AdicAval.Numero_Notaria;
                }

            }
            catch
            {
                //no se encuentra info de referencia notarial
            }

            try
            {
                var rn_poder = beRefNotariales.Obtener(int.Parse(cre.IdCliente.ToString()), 6);

                txtRepresentative_LegalDocumentNumber.Text =
                    rn_poder.Numero_Escritura;

                if (rn_poder.Fecha_Escritura != null &&
                    rn_poder.Fecha_Escritura.ToString() != "")
                {
                    txtRepresentative_LegalDocumentFecha.Text =
                        Convert.ToDateTime(rn_poder.Fecha_Escritura.ToString()).ToString("yyyy-MM-dd");
                }

                txtRepresentative_NotariesProfessionName.Text =
                    rn_poder.Nombre_Notario;
                txtRepresentative_NotariesProfessionNumber.Text =
                    rn_poder.Numero_Notaria;

                if (rn_poder.Estado != null)
                {
                    drpRepresentative_Estado.SelectedValue =
                        rn_poder.Estado.ToString();
                    RepresentativeDelegMunicipioBind();
                }

                if (rn_poder.Municipio != null)
                {
                    drpRepresentative_OfficeorMunicipality.SelectedValue =
                        rn_poder.Municipio.ToString();
                }
            ...
            try
            {
                var rn_acta = beRefNotariales.Obtener(int.Parse(cre.IdCliente.ToString()), 7);

                txtApplicant_LegalDocumentNumber.Text =
                    rn_acta.Numero_Escritura;

                if (rn_acta.Fecha_Escritura != null && rn_acta.Fecha_Escritura.ToString() != "")
                {
                    txtApplicant_LegalDocumentFecha.Text =
                        Convert.ToDateTime(rn_acta.Fecha_Escritura.ToString()).ToString("yyyy-MM-dd");
                }

                txtApplicant_NotariesProfessionName.Text =
                    rn_acta.Nombre_Notario.ToString();
                txtApplicant_NotariesProfessionNumber.Text =
                    rn_acta.Numero_Notaria.ToString();

                if (rn_acta.Estado != null)
                {
                    drpApplicant_Estado.SelectedValue =
                        rn_acta.Estado.ToString();
                    ApplicantDelegMunicipioBind();
                }

                if (rn_acta.Municipio != null)
                {
                    drpApplicant_OfficeOrMunicipality.SelectedValue =
                        rn_acta.Municipio.ToString();
                }
            ...
            DataTable dtCreditoSustitucion =
                K_CREDITO_SUSTITUCIONDAL.ClassInstance.Get_K_CREDITO_SUSTITUCIONByNo_Credito(CreditNumber);
            if (dtCreditoSustitucion != null && dtCreditoSustitucion.Rows.Count > 0)
            {
                ...
                foreach (DataRow dataRow in dtCreditoSustitucion.Rows)
                {
                    ...
                    if (ktm.GetMaterialMaxOrderByTechnology(Convert.ToInt32(dataRow["Technology"].ToString())) > 0)
                    {
                        ...
                        // Added by Tina 2011/09/01
                        row["TypeOfProduct"] = dataRow["TypeOfProduct"] != DBNull.Value
                            ? dataRow["TypeOfProduct"].ToString()
                            : "";
                        ...
                        row["Antiguedad"] = dataRow["Antiguedad"] != DBNull.Value
                            ? dataRow["Antiguedad"].ToString()
                            : "";
                        row["DisposalType"] = dataRow["DisposalType"] != DBNull.Value
                            ? dataRow["DisposalType"].ToString()
                            : "";
                        if (dataRow["DisposalType"] != DBNull.Value && dataRow["DisposalType"].ToString() == "M")
                        {
                            row["DisposalCenter"] = dataRow["DisposalCenter"] != DBNull.Value
                                ? dataRow["DisposalCenter"].ToString() + "-" + "(Matriz)"
                                : ""; // updated by tina 2012-02-29
                        }
                        else
                        {
                            row["DisposalCenter"] = dataRow["DisposalCenter"] != DBNull.Value
                                ? dataRow["DisposalCenter"].ToString() + "-" + "(Sucursal)"
                                : ""; // updated by tina 2012-02-29
                        }
        ...
        private void InitMopValidation()
        {
            ...
            if (dt2.Rows.Count > 0 && dt2.Rows[0][0] != DBNull.Value && Convert.ToInt16(dt2.Rows[0][0]) > 0 && Convert.ToInt16(dt2.Rows[0][1]) == 0)
            {
                btnDisplayEquipmentAct.Enabled = true;
                btnDisplayDisposalBonusReceipt.Enabled = true;
                btnDisplayReceiptToSettle.Enabled = true;
        ...
        {
            if (PAEEEM.LogicaNegocios.AltaBajaEquipos.InfoCompAltaBajaEquipos.TieneFoto(CreditNumber) == true)
            {
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "PrintForm", "window.open('PrintForm.aspx?ReportName=EquipoBajaEficienciaSinFoto&CreditNumber=" + CreditNumber + "','','top=0,left=0,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no');", true);
            }
        }
