 ...
 ** ******************************** **/
	...
	if(isset($_POST['action']) && $_POST['action'] == 'contact_send') {
		...
		foreach($post_data as $key=>$value) {
			...
			*/
			...
			if(isset($post_data[$key]['required'])) {
				...
				if($key == 'nombre')
					$array['nombre'] = $post_data[$key]['required'];
		...
		// ATTACHMENT
		if(isset($_FILES['contact']['nombre']['attachment']) && $_FILES['contact']['nombre']['attachment'] != '') { // name|type|tmp_name|error|size|

			$target_file 	= UPLOAD_FOLDER . basename($_FILES['contact']['nombre']['attachment']);
			$rename 		= time() . '_' . basename($_FILES['contact']['nombre']['attachment']);
			$uploadOk 		= 1;
			$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);

			// Check if image file is a actual image or fake image
			if(isset($_POST["submit"])) {
				$check = getimagesize($_FILES['contact']['tmp_nombre']['attachment']);
				if($check !== false) {
			...
			// if everything is ok, try to upload file
				...
				if (move_uploaded_file($_FILES['contact']['tmp_nombre']['attachment'], UPLOAD_FOLDER . $rename)) {
		...
		if($config['use_smtp'] === true) {
			...
			$m->AddReplyTo($array['email'],   isset($array['nombre']) ? $array['nombre'] : $array['email']);
			$m->SetFrom($config['smtp_user'], isset($array['nombre']) ? $array['nombre'] : $array['email']);
			$m->MsgHTML("
				{$email_body}
				---------------------------------------------------<br>
				IP: {$ip}
