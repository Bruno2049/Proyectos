...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id('FK_ProProducto_ProFamililia') AND OBJECTPROPERTY(id, 'IsForeignKey') = 1)
ALTER TABLE ProProducto DROP CONSTRAINT FK_ProProducto_ProFamililia
;
...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id('ProFamililia') AND  OBJECTPROPERTY(id, 'IsUserTable') = 1)
DROP TABLE ProFamililia
;
...
ALTER TABLE ProFamililia ADD CONSTRAINT PK_ProFamililia 
...
ALTER TABLE ProProducto ADD CONSTRAINT FK_ProProducto_ProFamililia 
	FOREIGN KEY (IdFamilia) REFERENCES ProFamililia (IdFamilia)
;
